...
public final class CertifiedBundleUpdater {
    ...
    public static void maybeUpdate(Context context, File jsonFile) {
        final int remoteVersion;
        try {
            remoteVersion = getRemoteVersion(jsonFile);
    ...
    private static int getRemoteVersion(File jsonFile) throws IOException, JSONException {
        JSONObject root = new JSONObject(Files.readString(jsonFile.toPath()));
        ...
        for (int i = 0; i < response.length(); i++) {
            ...
            version = getCertifiedVersion(update);
            if (version > 0) {
                return version;
            }
        }
